...
func generate_seed_string(string_length: int = DEFAULT_LENGTH) -> String:
...
func normalize_seed_string(s: String, length: int = DEFAULT_LENGTH) -> String:
	var clean := sanitize_seed_string(s)
	if clean.length() == length:
		return clean
	# If too short or invalid, generate fresh seed
	return generate_seed_string(length)
